...
```
The mean number of steps taken per day is `r round(aggmean,4)` and the median number of steps taken per day is `r round(aggmed,4)`.
...
mxsteps
```
We can then see that the interval with the maximum number of steps is interval number `r mxsteps[1,1]`.
...
aggmean <- mean(agg$steps);aggmed <- median(agg$steps)
```
The mean number of steps taken per day, for the dataset with the NAs filled in with means, is `r round(aggmean,4)` and the median number of steps taken per day is `r round(aggmed,4)`.  Performing the substituion appears to make very little difference - it is noted that the mean and the median are exactly the same.  The histogram has a a very similar shape, although the central bar is higher.
